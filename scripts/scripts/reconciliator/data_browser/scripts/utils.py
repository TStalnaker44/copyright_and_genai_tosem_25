...
class Browser():
    ...
    def __init__(self):
        self.qdata = readJSON(os.path.join("surveys", "ai_legal", "questions.json"))
        self.rdata = readJSON(os.path.join("surveys", "ai_legal", "files", "results.json"))
        self.question_mapping = self.getQuestionMapping()
        most_recent = getMostRecent(os.path.join("surveys", "ai_legal"))
